...
def _make_profile_command(kernel: str):
    """
    Gets the command to use for profiling, sans the actual program invocation
    """
    # TODO check if we're AMD or NVIDIA
    return ["rocprof-compute", "profile", "-k", kernel, "-n", "kb-prof", "--"]
...
def run_single_evaluation(
    ...
    if mode == "test":
        ...
        # TODO allow more flexible benchmarking
        timeout = 120
        with tempfile.NamedTemporaryFile("w") as bench_file:
            bench_file.write(benchmarks.splitlines(keepends=True)[-1])
            bench_file.flush()
            result = run_program(_make_profile_command(kernel) + call + [mode, bench_file.name], seed=seed,
                                 timeout=timeout)
            if not result.success:
                return result

            prof_command = [
                "rocprof-compute", "analyze", "-p", "workloads/kb-prof/MI300X_A1/",
                "-b", "2", "3", "5", "7", "10", "11", "12", "14", "15", "16", "17",
                "-o", "report.txt"
            ]
            run_process = subprocess.run(
                prof_command,
            ...
            result.command += " && " + " ".join(prof_command)
            result.stderr += "\n\n" + run_process.stderr
            result.stdout += "\n\n" + run_process.stdout

            profile_data = Path("report.txt").read_text()

            result.stdout += profile_data
            return result
    else:
        assert mode == "script"
